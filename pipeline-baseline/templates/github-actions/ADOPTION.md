...
| `ref` | `codex/runtime-governance-addendum` | Governance branch/tag/SHA to use. |
| `--baseline` | `app-demo-baseline` | Solution baseline identifier expected for release compatibility. |

For stable demos, use a fixed governance branch or tag. For production use, prefer a version tag or pinned commit SHA.
